...
void TcpLola::IncreaseWindow (Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
  ...
  switch(m_nextState){
   case NS_SLOW_START : callSlowStart(tcb,segmentsAcked);break;
   case	NS_CUBIC      : callCubic(tcb, segmentsAcked);break;       
   case	NS_FAIR_FLOW  : callFairFlow(tcb, segmentsAcked);break;
   case	NS_CWND_HOLD  :  callCwndHold(tcb,segmentsAcked);break;
   case	NS_TAIL_DECREASE : callTailDecrease(tcb,segmentsAcked);break;
  }
  m_cwndMaxTemp = std::max (static_cast<uint32_t>(tcb->m_cWnd), m_cwndMaxTemp);
}
